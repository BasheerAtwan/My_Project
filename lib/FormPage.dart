...
class _FormPageState extends State<FormPage> {
  ...
  Future<void> _saveFormDataToFirebase() async {
    try {
      ...
      Map<String, dynamic> data = {
        ...
        'imageUrls': imageUrls, // Include image URLs even if empty
      };
              ...
              TextButton(
                onPressed: () {
                  ...
                  setState(() {
                    ...
                    _selectedImageBytesList.clear(); // Clear image list
                  });
                  Navigator.of(context).pop();
                },
                child: Text('نعم'),
  ...
  Future<List<String>> _uploadImagesToFirebase() async {
    ...
    for (int i = 0; i < _selectedImageBytesList.length; i++) {
      String uniqueFileName = Uuid().v4(); // Generate a unique UUID
      Reference referenceDirImages = FirebaseStorage.instance.ref().child('images');
      Reference referenceImageToUpload = referenceDirImages.child(uniqueFileName);

      try {
        // Set the content type based on the image format (jpeg or png)
        String contentType = 'image/jpeg'; // You may need to determine the format dynamically
